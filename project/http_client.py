...
class AilyHTTPClient:
    ...
    async def create_task(
        ...
        resp = await self.client.post(
            url,
            headers=self._headers(svc_method="aily_workbench_CreateTask"),
            json=body,
        )
    ...
    async def send_comment(
        ...
        body = {
            ...
            or f"py_server_{int(time.time() * 1000)}",
            "notable": notable,
            "mentions": mentions or [],
        }
        resp = await self.client.post(
            url,
            headers=self._headers(svc_method="aily_workbench_CreateComment"),
            json=body,
        )
    ...
    async def fetch_events(self, session_id: str, page_size: int = 200) -> list:
        """拉取 session 的事件列表"""
        url = (
            f"{BASE_URL}{API_PREFIX}/sessions/{session_id}/events?pageSize={page_size}"
        )
    ...
    async def fetch_ticket(self) -> dict:
        ...
        resp = await self.client.post(
            url,
            headers=self._headers(svc_method="FrontierBackService_GetSessionTicket"),
            json={},
        )
    ...
    async def _check(self, resp: httpx.Response, op: str):
        ...
        if resp.status_code in (302, 401):
            raise CookieExpiredError(
                f"Cookie 已过期（{op}），请运行 update_cookie.py 更新"
            )

...
async def health(request: Request):
    ...
    return {
        ...
        "ticket_expires_at": ws.ticket_data.get("expiresAt")
        if ws and ws.ticket_data
        else None,
    ...
    if msg.auto_watch:
        try:
            session_id = result.get(
                "sessionId"
            ) or await state.http_client.get_session_id(msg.task_id)
            if session_id:
                response["session_id"] = session_id
...
async def stop_watch(req: StopRequest, request: Request):
    ...
    state = request.app.state.server
    watcher = (
        state.ws_manager.watchers.pop(req.session_id, None)
        if state.ws_manager
        else None
    )
    if watcher:
...
async def event_stream(session_id: str, request: Request):
    ...
    if not watcher:
        raise HTTPException(
            status_code=404, detail="未找到该 session 的监听，请先 POST /api/watch"
        )

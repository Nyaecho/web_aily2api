...
async def chat_completions(req: OpenAIChatRequest, request: Request):
    ...
    if not session_id:
        raise HTTPException(status_code=504, detail=f'任务已创建 ({task_url}) 但未生成 session_id')

    # 启动 watch
    watcher = await state.auto_watch_session(session_id, task_id)

    log.info(f'OpenAI chat: task={task_id} session={session_id} model={req.model} stream={req.stream}')

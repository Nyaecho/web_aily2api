...
async def openai_stream(
    watcher: SessionWatcher, model_name: str, agent_id: str, user_content: str
    ...
    while time.time() < deadline:
        ...
        if etype == "message.update":
            ...
            if content and content != sent_content:
                delta_text = (
                    content[len(sent_content) :]
                    if content.startswith(sent_content)
                    else content
                )
                if delta_text:
                    chunk = {
